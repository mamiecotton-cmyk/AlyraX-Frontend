...
function styleForMediaPrompt(prompt: string) {
  return /\b(current live voice-call context|doing right now|what('re| are| is|’re) (you|u|he|she|they) wear(?:ing)?|wearing right now|full body|full-body|fullbody|full length|full-length|head to toe|head-to-toe|entire body|whole body|body shot|outfit|fit check|clothes|clothing|wardrobe|legs?|standing|spread)\b/i.test(prompt)
    ? 'fullbody'
    : 'portrait';
}
