...
function isShowerPrompt(prompt: string) {
  return /\b(shower|showering|running water|wet skin|water droplets|bathroom stall|steam)\b/i.test(prompt);
}
...
function buildFinalPrompt(prompt: string, triggerWord: string, style: ImageStyle, characterId?: string) {
  ...
  const triggerPhrase = characterId === 'jaxon' ? `${triggerWord}, ${triggerWord}` : triggerWord;
  const promptParts = sceneAnchor || characterId === 'jaxon'
    ? [triggerPhrase, characterAnchor, sceneAnchor, posePhrase, promptWithoutTrigger, PHOTOREALISM_PROMPT, composition]
    : [posePhrase, promptWithoutTrigger, triggerPhrase, characterAnchor, PHOTOREALISM_PROMPT, composition];
